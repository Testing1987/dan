...
namespace Alignment_mdi
{
    class Functions
    {
        ...
        public static Polyline Build_2d_poly_for_scanning(System.Data.DataTable dt_cl)
        {
            ...
            {
                MessageBox.Show("no centerline loaded", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
        ...
        public static int get_excel_column_number(string columnName)
        {
        ...
        public static void Transfer_datatable_to_existing_excel_spreadsheet(Worksheet W1, System.Data.DataTable dt1, string col_start, int row_start, bool transfer_column_names = false)
        {
            if (dt1 != null && W1 != null)
            {
                if (dt1.Rows.Count > 0 && dt1.Columns.Count > 0)
                {

                    int maxRows = dt1.Rows.Count;
                    int maxCols = dt1.Columns.Count;

                    int index_col_start = get_excel_column_number(col_start);
                    int index_last_col = index_col_start + maxCols - 1;

                    string last_col = get_excel_column_letter(index_last_col);


                    Range range1 = W1.Range[col_start + row_start.ToString() + ":" + last_col + (row_start + maxRows - 1).ToString()];
                    ...
                    if (transfer_column_names == true)
                    {
                        for (int i = 0; i < dt1.Columns.Count; ++i)
                        {
                            W1.Cells[row_start - 1, i + 1].value2 = dt1.Columns[i].ColumnName;
                        }
                    }
        ...
        public static DataGridViewTextBoxColumn datagrid_textbox_column(string col_name)
        {
        ...
        public static DataGridViewCheckBoxColumn datagrid_checkbox_column(string col_name)
        {
        ...
        public static List<string> get_blocks_from_current_drawing()
        {
        ...
        public static void add_object_data_to_datatable(System.Data.DataTable dt1, Autodesk.Gis.Map.ObjectData.Tables Tables1, ObjectId id1)
        {
