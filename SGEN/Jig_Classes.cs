...
namespace Alignment_mdi
{
    ...
    class Jig_show_rectangle_top_left : DrawJig
    {
        ...
        public PromptPointResult StartJig(double VVw_Scale, double VVw_width, double VVw_height, string _anchor)
        {
        ...
        protected override SamplerStatus Sampler(JigPrompts prompts)
        {
            PromptPointResult1 = prompts.AcquirePoint("\nSpecify the rectangle location:");



            if (PromptPointResult1.Value.IsEqualTo(Punct1))
            {
                return SamplerStatus.NoChange;
            }
            else
            {
                Punct1 = PromptPointResult1.Value;
                return SamplerStatus.OK;
            }
        ...
        {
            Polyline Poly1 = new Autodesk.AutoCAD.DatabaseServices.Polyline();
            Poly1 = creaza_rectangle_viewport(Punct1);
            draw.Geometry.Polyline(Poly1, 0, 4);
            return true;
        }

        private Polyline creaza_rectangle_viewport(Point3d Point1)
        {

            Polyline Poly1r = new Autodesk.AutoCAD.DatabaseServices.Polyline();
            if (anchor == "TL")
            {
                Poly1r.AddVertexAt(0, new Point2d(Point1.X, Point1.Y), 0, 0, 0);
                Poly1r.AddVertexAt(1, new Point2d(Point1.X + Vw_width / Vw_scale, Point1.Y), 0, 0, 0);
                Poly1r.AddVertexAt(2, new Point2d(Point1.X + Vw_width / Vw_scale, Point1.Y - Vw_height / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(3, new Point2d(Point1.X, Point1.Y - Vw_height / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(4, new Point2d(Point1.X, Point1.Y), 0, 0, 0);
            }
    ...
    class Jig_show_rectangle_with_JigPromptPointOptions : DrawJig
    {
        ...
        public PromptPointResult StartJig(double VVw_Scale, double VVw_width, double VVw_height, string _anchor, bool is_plat = false)
        {
            ...
            if (PromptPointResult1.Status == PromptStatus.None)
            {
                return null;
            }
        ...
        protected override SamplerStatus Sampler(JigPrompts prompts)
        {
            JigPromptPointOptions prompt2 = new JigPromptPointOptions("\nSelect insertion point:");
        ...
        private Polyline creaza_rectangle_viewport(Point3d Point1)
        {
            if (izzz_plat == true)
            {
                Vw_scale = _SGEN_mainform.tpage_sheetindex.forma3. get_current_scale();
            }
            ...
            if (anchor == "CEN")
            {
                Poly1r.AddVertexAt(0, new Point2d(Point1.X - Vw_width * 0.5 / Vw_scale, Point1.Y - Vw_height * 0.5 / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(1, new Point2d(Point1.X - Vw_width * 0.5 / Vw_scale, Point1.Y + Vw_height * 0.5 / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(2, new Point2d(Point1.X + Vw_width * 0.5 / Vw_scale, Point1.Y + Vw_height * 0.5 / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(3, new Point2d(Point1.X + Vw_width * 0.5 / Vw_scale, Point1.Y -Vw_height * 0.5 / Vw_scale), 0, 0, 0);
                Poly1r.AddVertexAt(4, new Point2d(Point1.X - Vw_width * 0.5 / Vw_scale, Point1.Y - Vw_height * 0.5 / Vw_scale), 0, 0, 0);
