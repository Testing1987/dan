...
namespace Alignment_mdi
{
    public partial class Wgen_main_form : Form
    {
        ...
        public static Wgen_dismiss_errors tpage_dismiss_errors = null;
        ...
        public static System.Data.DataTable dt_pipe_manifest = null;
        ...
        public static string wmr2 = "wgen_wm_R2.xlsx";
        static public bool check_dj_against_xray = false;
        static public bool use_pmc_as_cc = false;
        public static bool dj_total_length = false;
        ...
        public Wgen_main_form()
        {
            ...
            dt_feature_codes.Columns.Add("CHECK DJ AGAINST XRAY", typeof(string));
            dt_feature_codes.Columns.Add("DJ TOTAL LENGTH", typeof(bool));
            dt_feature_codes.Columns.Add(col_pmc_as_cc, typeof(bool));
            ...
            if (System.IO.File.Exists(file1) == true)
            {
                ...
                try
                {
                    ...
                    foreach (Microsoft.Office.Interop.Excel.Worksheet W1 in Workbook1.Worksheets)
                    {
                        ...
                        Microsoft.Office.Interop.Excel.Range range2 = W1.Range["A2: X30000"];
                        object[,] values2 = new object[30000, 24];
                        values2 = range2.Value2;
                        object valV = values2[1, 22];
                        bool add_valV = true;
                        object valW = values2[1, 23]; // JUST CELL W2

                        bool client_dj = false;

                        if (valW != null)
                        {
                            string val1 = Convert.ToString(valW);

                            if (val1.ToUpper().Replace(" ", "") == "YES" || val1.ToUpper().Replace(" ", "") == "TRUE")
                            {
                                if (index_sheet == 1)
                                {
                                    dj_total_length = true;
                                }
                                client_dj = true;
                            }
                            else
                        ...
                        for (int i = 1; i <= values2.Length / 3; ++i)
                        {
                            ...
                            if (valA != null)
                            {
                                ...
                                if (add_valV == true)
                                {
                                    if (valV != null)
                                    {
                                        string val1 = Convert.ToString(valV);

                                        if (val1.ToUpper().Replace(" ", "") == "YES" || val1.ToUpper().Replace(" ", "") == "TRUE")
                                        {
                                            dt_feature_codes.Rows[dt_feature_codes.Rows.Count - 1]["CHECK DJ AGAINST XRAY"] = "YES";
                                        }
                                        else
                                        {
                                            dt_feature_codes.Rows[dt_feature_codes.Rows.Count - 1]["CHECK DJ AGAINST XRAY"] = "NO";
                                        }
                                    }
                                    else
                                    {
                                ...
                                dt_feature_codes.Rows[dt_feature_codes.Rows.Count - 1]["DJ TOTAL LENGTH"] = client_dj;

                                if(Convert.ToString( valA)== "PIPE_MATERIAL_CHANGE" && valX != null)
                                {
                                    if (Convert.ToString(valX).ToUpper().Replace(" ", "") == "YES" || Convert.ToString(valX).ToUpper().Replace(" ", "") == "TRUE")
                                    {
                                        dt_feature_codes.Rows[dt_feature_codes.Rows.Count - 1][col_pmc_as_cc] = true;
                                        if (index_sheet == 1)
                                        {
                                            use_pmc_as_cc = true;
                                        }
                                    ...
                                    {
                                        dt_feature_codes.Rows[dt_feature_codes.Rows.Count - 1][col_pmc_as_cc] = false;
                                        if (index_sheet == 1)
                                        {
                                            use_pmc_as_cc = false;
                                        }
                        ...
                        if (index_sheet == 1 && valV != null)
                        {
                            string val1 = Convert.ToString(valV);

                            if (val1.ToUpper().Replace(" ", "") == "YES" || val1.ToUpper().Replace(" ", "") == "TRUE")
                            {
                                check_dj_against_xray = true;
                            }
                            else
                            {
                                check_dj_against_xray = false;
                            }
                        }
            ...
            tpage_dismiss_errors = new Wgen_dismiss_errors();
            tpage_dismiss_errors.MdiParent = this;
            tpage_dismiss_errors.Dock = DockStyle.Fill;
            tpage_dismiss_errors.Hide();

           tpage_weldmap.set_button_gen_wmR2();
        ...
        private void button_Exit_Click(object sender, EventArgs e)
        {
            ...
            dt_pt_move = null;
            client_name = "xxx";
            tpage_pipe_tally.dt_dismissed_errors = null;
            tpage_pipe_manifest.dt_dismissed_errors = null;
            tpage_allpts.dt_dismissed_errors = null;
            this.Close();
        }
        ...
        private void treeView_inquiry_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (e.Node.Text == "Dismissed Errors")
            {
                tpage_pipe_manifest.Hide();
                tpage_pipe_tally.Hide();
                tpage_weldmap.Hide();
                ...
                tpage_build_pipe_tally.Hide();
                tpage_duplicates.Hide();
                tpage_dismiss_errors.Show();

                tpage_dismiss_errors.set_label1_text("Pipe Manifest");

                tpage_dismiss_errors.display_dt_to_datagridView(tpage_pipe_manifest.dt_dismissed_errors);
        ...
        private void radioButton1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            Font enlargedFont = new Font("Arial", 10f, FontStyle.Bold);
            ...
            tpage_allpts.radioButton_all_points_CheckedChanged(radioButton2);
            tpage_pipe_manifest.radioButton_pipe_manifest_CheckedChanged(radioButton2);
            tpage_pipe_tally.radioButton_pipe_tally_CheckChanged(radioButton2);
            tpage_duplicates.radioButton_duplicates_CheckChanged(radioButton2);
            tpage_weldmap.radioButton_weldmap_CheckChanged(radioButton2);
            tpage_dismiss_errors.radioButton_dismiss_errors_CheckedChanged(radioButton2);

            if (radioButton1.Checked == true)
            {
                treeView_inquiry.Font = new Font("Arial", 10f, FontStyle.Bold);
            }
            else
            {
                treeView_inquiry.Font = new Font("Arial", 12f, FontStyle.Bold);
            }
            ...
            {
                label12.Font = enlargedHeader;
            }

            if (radioButton1.Checked == true)
            {
                radioButton1.Font = regularfont;
            }
            else
            {
                radioButton1.Font = enlargedFont;
            }

            if (radioButton1.Checked == true)
            {
                radioButton2.Font = regularfont;
            }
            else
            {
                radioButton2.Font = enlargedFont;
            }
